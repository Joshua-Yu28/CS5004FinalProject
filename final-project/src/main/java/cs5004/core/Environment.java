...
public class Environment<V> {
  ...
  /**
   * Create an environment in which the names are associated with the
   * values, pairwise.  (The first name is associated with the first value,
   * and so on.)
   ...
   * plus the one specified by the arguments.  (If the current environment
   * already has a value for name, the new binding will override this in
  ...
  public Environment<V> extend(String name, V value) {
    return new Environment<V>(Map.of(name, value), this);
  }
